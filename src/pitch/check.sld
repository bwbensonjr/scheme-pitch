...
;;   "\x41;" as "A", #\nul as #\null
;;
;; (pitch format) wires both layers to the text emitted by the printer and
;; withholds output if either fresh-text comparison fails.
...
(begin
  ...
  ;; an absent side.
  (define-record-type <mismatch> (make-mismatch index input-token
  ...
  ;; divergence has somewhere to live. Today it is the supported Emit revision's
  ;; cycle-safe equal?. Opaque numeric vectors compare by their shared private
  ;; marker and retained lexeme, while remaining unequal to every ordinary datum
  ...
  ;; formatter more than once. Both are exercised at the parser/formatter edges.
