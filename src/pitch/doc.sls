...
(library (pitch doc)
  (export
    ...
    doc? doc-nl-cnt fullness-index doc-fails-statically?
    doc-text? doc-text-len doc-text-push
    doc-newline? doc-newline-s
...
(define (doc-fails-statically? d index)
  (bitwise-bit-set? (doc-fail-mask d) index))
...
(define (newline s)
  (make-doc-newline fail-newline 1 s))
...
(define (concat a b)
  (cond ((and (doc-text? a) (= 0 (doc-text-len a))) b)
...
(define (nest n d)
  (cond ((indentation-inert? d) d)
        ((doc-nest? d) (nest (+ n (doc-nest-n d)) (doc-nest-d d)))
        (else (make-doc-nest fail-none (doc-nl-cnt d) n d))))

(define (align d)
  (if (indentation-inert? d)
      d
      (make-doc-align fail-none (doc-nl-cnt d) d)))

(define (reset d)
  (if (indentation-inert? d)
      d
      (make-doc-reset fail-none (doc-nl-cnt d) d)))
...
(define (cost n d)
  (if (doc-fail? d)
      fail
      (make-doc-cost fail-none (doc-nl-cnt d) n d)))
...
(define (alt . xs)
  (fold-right alternatives fail xs))
