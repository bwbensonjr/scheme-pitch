...
(begin
  (define (for-all predicate values)
    (if (null? values)
        #t
        (and (predicate (car values)) (for-all predicate (cdr values)))))

  (define (exists predicate values)
    (if (null? values)
        #f
        (or (predicate (car values)) (exists predicate (cdr values)))))
  ...
  (define (fold-right combine initial values)
    (if (null? values)
        initial
        (combine (car values) (fold-right combine initial (cdr values)))))
  ...
  (define (merge less? left right)
    (cond
      ((null? left) right)
      ((null? right) left)
      ((less? (car right) (car left)) (cons (car right) (merge less? left (cdr right))))
      (else (cons (car left) (merge less? (cdr left) right)))))
  ...
  (define (list-sort less? values)
    (if (or (null? values) (null? (cdr values)))
        ...
        (call-with-values
          ...
          (lambda (left right)
            (merge less? (list-sort less? left) (list-sort less? right))))))))
