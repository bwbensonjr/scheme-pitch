...
(begin
  ...
  (define-record-type <cost-factory> (make-cost-factory cost<=? cost+ cost-text cost-nl
                                      limit) cost-factory?
    (cost<=? cost-factory-cost<=?)
    (cost+ cost-factory-cost+)
    (cost-text cost-factory-cost-text)
    (cost-nl cost-factory-cost-nl)
    (limit cost-factory-limit))
  ...
  ;; what `div` computes.
  (define (default-computation-width page-width) (quotient (* page-width 6) 5))
