...
(library (pitch layout)
  (export
    layout pretty-format
    layout-result? layout-result-tainted? layout-result-cost
    layout-failure?)
  (import
    (rnrs base (6))
    (rnrs control (6))
    ...
    (pitch cost)
    (only (pitch doc)
          doc-nl-cnt fullness-index doc-fails-statically?
          doc-text? doc-text-len doc-text-push
          doc-newline?
          doc-concat? doc-concat-a doc-concat-b
          doc-alt? doc-alt-a doc-alt-b
          doc-nest? doc-nest-n doc-nest-d
          doc-align? doc-align-d
          doc-reset? doc-reset-d
          doc-full? doc-full-d
          doc-cost? doc-cost-n doc-cost-d
          doc-fail?))
...
(define-condition-type &layout-failure &error
  make-layout-failure layout-failure?)
      ...
      (if (null? ps)
          ...
          (let* ((s (car ps)) (n (string-length s)))
            (let copy ((k 0))
              (if (= k n)
                  (loop (cdr ps) (+ at n))
                  (begin (string-set! out (+ at k) (string-ref s k))
                         (copy (+ k 1))))))))))

(define (render m)
  (concatenate (reverse ((measure-tokens m) '()))))
  ...
  (let* ((cost<=? (cost-factory-cost<=? factory))
    ...
    (define (known-failing? d index)
      (or (doc-fails-statically? d index)
          (bitwise-bit-set? (hashtable-ref dynamic-failure d 0) index)))

    (define (note-failing! d index)
      (hashtable-set! dynamic-failure d
                      (bitwise-ior (hashtable-ref dynamic-failure d 0)
    ...
    (define (extend-left d a-m b-ms)
      (cond
        ...
        ((null? b-ms) '())
        (else
         (let loop ((best (concat-measure a-m (car b-ms)))
                    (rest (cdr b-ms))
                    (kept '()))
           (if (null? rest)
    ...
    (define (resolve-form d c i beg-full? end-full?)
      (cond
        ...
        ((doc-alt? d)
         (merge (resolve (doc-alt-a d) c i beg-full? end-full?)
        ...
        ((doc-full? d)
         (merge (resolve (doc-full-d d) c i beg-full? #f)
    ...
    (let* ((result (merge (resolve d offset 0 #f #f)
                          (resolve d offset 0 #f #t)
                          #f))
           (tainted? (lazy-set? result))
           (final (extract-at-most-one result)))
      (if (null? final)
          (raise-layout-failure)
          (values (car final) tainted?)))))
  ...
  (case-lambda
    ...
    ((d factory offset)
     (let-values (((m tainted?) (resolve-document d factory offset)))
       (values (render m)
               (make-layout-result tainted? (measure-cost m)))))))
