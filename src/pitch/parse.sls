        ...
        (let loop ()
          (let ((tok (get-token reader)))
            ...
            (unless (null? pending)
              (for-each (lambda (msg)
                          (set! diagnostics
                                (cons (make-diagnostic msg tok) diagnostics)))
                        (reverse pending))
              (set! pending '()))
            (unless (eq? (token-kind tok) 'eof)
              (loop))))))
    (values (list->vector (reverse tokens))
            (reverse diagnostics))))
...
(define (expected-close open-kind)
  (if (eq? open-kind 'openb) 'closeb 'closep))
...
(define (parse-tokens tokens)
  (let ((n (vector-length tokens))
            ...
            ((close-kind? k)
             (if top?
                 (let ((msg "Unexpected closing delimiter"))
                   ...
                   (loop (+ i 1)
                         (cons (make-error-node (tok-at i) msg
                                                (list (make-leaf (tok-at i))))
                               acc)))
                 (values (reverse acc) i)))
            (else
             (let-values (((node j) (parse-item i)))
               (loop j (cons node acc))))))))

    (define (parse-item i)
      (let ((k (kind-at i)))
        (cond ((open-kind? k) (parse-compound i))
    ...
    (define (finish-compound open children close)
      (let ((node (make-compound open children close)))
        (check-dots! node)
        node))
    ...
    (define (check-dots! node)
      (let* ((data (datum-children node))
             (dots (filter dot-leaf? data)))
        (unless (null? dots)
          (cond
            ((not (list-node? node))
             (for-each (lambda (d)
                         (diagnose! "Dot is not allowed here" (leaf-token d)))
                       dots))
            ((not (null? (cdr dots)))
             (for-each (lambda (d)
                         (diagnose! "More than one dot in a list"
                                    (leaf-token d)))
                       dots))
            (else
             (let ((k (position-of (car dots) data))
                   (len (length data))
                   (dot-tok (leaf-token (car dots))))
               (when (= k 0)
                 (diagnose! "Dot with no datum before it" dot-tok))
               (unless (= len (+ k 2))
      ...
      (let ((marker-tok (tok-at i)))
        (let loop ((j (+ i 1)) (triv '()))
          (let ((k (kind-at j)))
            (cond
              ((trivia-kind? k)
               (loop (+ j 1) (cons (make-leaf (tok-at j)) triv)))
              ((or (eq? k 'eof) (close-kind? k))
               (diagnose! "Prefix with no datum" marker-tok)
               (values (make-prefix (make-leaf marker-tok) (reverse triv) #f)
                       j))
              (else
               (let-values (((datum j) (parse-item j)))
                 (values (make-prefix (make-leaf marker-tok)
                                      (reverse triv)
                                      datum)
                         j))))))))

    (assert (and (> n 0) (eq? (kind-at (- n 1)) 'eof)))
