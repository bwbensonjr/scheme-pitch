...
(library (pitch parse)
(export
  tokenize parse-tokens parse-source
  ...
  diagnostic? diagnostic-message diagnostic-token diagnostic-line diagnostic-column)
(import
  (rnrs base (6))
  (rnrs control (6))
  (rnrs lists (6))
  ...
  (pitch cst)
  (pitch diagnostic)
  (only (pitch reader) make-reader get-token token-kind reader-tolerant?-set!))
    ...
    (with-exception-handler
      (lambda (con)
        (if
          (warning? con)
          (set! pending
            (cons (if (message-condition? con) (condition-message con) "Lexical error")
                  pending))
          (raise con)))
      (lambda ()
        (let loop ()
      ...
      (let loop ((i i) (acc '()))
        (let ((k (kind-at i)))
          (cond
            ...
            (else (let-values (((node j) (parse-item i))) (loop j (cons node acc))))))))
          ...
          (let ((k (kind-at j)))
            (cond
              ...
              (else (let-values (((datum j) (parse-item j)))
                      (values (make-prefix (make-leaf marker-tok) (reverse triv) datum)
                              j))))))))
