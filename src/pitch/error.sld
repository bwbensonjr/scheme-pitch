...
(export
  ...
  reader-error-column reader-error-irritants make-layout-error layout-error?
  pitch-error? pitch-error-message)
(import (scheme base))
(begin
  (define-record-type <config-error> (make-config-error path message
                                      irritants) config-error?
    (path config-error-path)
    (message config-error-message)
    (irritants config-error-irritants))

  (define-record-type <reader-error> (make-reader-error filename line column message
                                      irritants) reader-error?
  ...
  (define-record-type <layout-error> (make-layout-error message) layout-error?
    (message layout-error-message))

  (define (pitch-error? value)
    (or (config-error? value) (reader-error? value) (layout-error? value)))
