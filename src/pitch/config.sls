...
(import
  ...
  (pitch parse)
  (pitch datum)
  (only (pitch style) style->shape make-style-table extend-style-table))

;;; Errors

(define-condition-type &config
                       &condition
                       make-config-condition
                       config-condition?
                       (path config-condition-path))

(define (bad path message . irritants)
  (raise (condition (make-config-condition path)
...
(define-record-type config-overlay
  (fields path width dialect common r6rs r7rs)
  (sealed #t)
  (opaque #t)
  (nongenerative config-overlay-v0-4d74a5f2-655a-49a9-b731-f062768d70a3))

;; Construction is private, so an invalid width, dialect or missing table
;; cannot cross the formatting boundary.
(define-record-type (resolved-config make-resolved-config resolved-config?)
  (fields width dialect common r6rs r7rs)
  (sealed #t)
  (opaque #t)
  (nongenerative resolved-config-v0-f4964c37-744f-4b15-ad11-36c6ad6aef34))
...
(define (config-style-table config dialect)
  (case dialect
    ...
    (else (assertion-violation 'config-style-table "Not a dialect" dialect))))
      ...
      (let ((entry (car entries)))
        ...
        (let ((heads (car entry)) (style (cadr entry)))
          (unless (and (pair? heads) (list? heads) (all-distinct-symbols? heads))
            (bad path "style heads must be a non-empty list of distinct symbols" heads))
          (for-each
            (lambda (head)
              (when (memq head seen)
