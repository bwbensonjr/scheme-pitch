...
(define (cst->datum document)
  (let ((diagnostics '())
    ...
    (define (diagnose! msg node)
      (set! diagnostics
            (cons (make-diagnostic msg (anchor-token node)) diagnostics)))
    ...
    (define (project node)
      (cond ((leaf? node) (project-leaf node))
            ((compound? node)
    ...
    (define (project-leaf node)
      (if (eq? (leaf-kind node) 'reference)
          (let ((n (token-value (leaf-token node))))
            (values (make-placeholder n) n))
    ...
    (define (build-chain triples)
      (if (null? triples)
          ...
          (let ((cells (map (lambda (t) (cons (car t) '())) triples)))
            (let link ((cs cells))
              (unless (null? (cdr cs))
                (set-cdr! (car cs) (cadr cs))
                (link (cdr cs))))
            (for-each (lambda (t cell)
                        (when (cadr t)
                          (register-patch! (cadr t) (caddr t)
                                           (lambda (v) (set-car! cell v)))))
                      triples cells)
            (car cells))))

    (define (last-cell chain)
      (if (null? (cdr chain)) chain (last-cell (cdr chain))))
          ...
          (let-values (((d ref) (project tail)))
            (unless (eq? d omitted)
              (let ((cell (last-cell chain)))
                (set-cdr! cell d)
                (when ref
                  (register-patch! ref tail
                                   (lambda (v) (set-cdr! cell v))))))))
        (values chain #f)))
    ...
    (define (project-vector node)
      (let* ((triples (project-each (remp dot-leaf? (projectable node))))
        ...
        (let loop ((ts triples) (i 0))
          (unless (null? ts)
            (when (cadr (car ts))
              (register-patch! (cadr (car ts)) (caddr (car ts))
    ...
    (define (project-prefix node)
      (let ((marker (prefix-marker node))
            (target (prefix-datum node)))
        (cond
          ((not target)
           (diagnose! "Prefix with no datum" marker)
           (values omitted #f))
    ...
    (define (project-label marker target)
      (let ((n (token-value (leaf-token marker))))
        (let-values (((d ref) (project target)))
          (cond ((eq? d omitted) (values omitted #f))
                (else
                 (if (hashtable-contains? labels n)
        ...
        (let-values (((d ref) (project target)))
          (if (eq? d omitted)
              ...
              (let* ((cell (cons d '()))
                     (form (cons sym cell)))
                (when ref
                  (register-patch! ref target (lambda (v) (set-car! cell v))))
                (values form #f))))))
