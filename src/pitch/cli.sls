...
(define (emit port . parts) (for-each (lambda (s) (put-string port s)) parts))
...
(define (report-at host path line column message)
  (report host path ":" (number->string line) ":" (number->string column) ": " message))
...
(define (emit-usage port) (for-each (lambda (line) (emit port line "\n")) usage-lines))
...
(define (walk-reversed host root)
  (let descend ((dir root) (acc '()))
    (let loop ((names (list-sort string<? ((host-list-directory host) dir))) (acc acc))
      (if (null? names)
          acc
          (let ((name (car names)))
            (cond
