...
(library (pitch cst)
  (export
    ...
    ;; interior nodes
    make-document document? document-children document-eof
    make-compound compound? compound-open compound-children compound-close
    compound-kind
    make-prefix prefix? prefix-marker prefix-children prefix-datum
    make-error-node error-node? error-node-token error-node-message
    error-node-children
    ;; generic
    cst-node? node-kind node-children
    list-node? vector-node? bytevector-node?
    ;; classification
...
(define (bytevector-node? x)
  (and (compound? x) (eq? (compound-kind x) 'bytevector)))
...
(define (trivia? node)
  (and (leaf? node)
       (memq (leaf-kind node)
             '(whitespace comment nested-comment inline-comment directive
               shebang))
       #t))
...
(define (dot-leaf? node)
  (and (leaf? node) (eq? (leaf-kind node) 'dot)))
...
(define (list-improper? node)
  (and (list-node? node)
       (let* ((data (datum-children node))
              (n (length data)))
         (let loop ((i 0) (xs data) (dot #f))
           (cond ((null? xs) (and dot (= n (+ dot 2))))
                 ((dot-leaf? (car xs))
                  (and (not dot)                ;a second dot is not a tail
...
(define (cst-leaves node)
  (define (walk node acc)                       ;acc is reversed
    (cond ((leaf? node) (cons node acc))
          ((document? node)
           (cons (document-eof node) (walk* (document-children node) acc)))
          ((compound? node)
           (let ((acc (walk* (compound-children node)
                             (cons (compound-open node) acc))))
             (if (compound-close node)
                 (cons (compound-close node) acc)
                 acc)))
          ((prefix? node)
           (let ((acc (walk* (prefix-children node)
                             (cons (prefix-marker node) acc))))
             (if (prefix-datum node)
                 (walk (prefix-datum node) acc)
                 acc)))
...
(define (cst-tokens node)
  (map leaf-token (cst-leaves node)))
...
(define (cst->text node)
  (call-with-string-output-port
    (lambda (port) (write-cst node port)))))
