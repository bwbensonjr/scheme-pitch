...
;; representable numbers, booleans and the empty list. A syntactically valid
;; number outside the host numeric tower remains the private opaque value
;; carried by its token. Comparing two projections uses the supported Emit
;; revision's cycle-safe equal?; the opaque value's private singleton marker
;; makes it unequal to every source-constructible vector.
;;
;; Every leaf's contribution is token-value, including an opaque number when
;; needed. There is no number parser, string unescaper or character-name
...
(begin
  ...
  (define-record-type <placeholder> (make-placeholder label) placeholder?
    ...
    (let ((diagnostics '())
      ...
      (define (project node)
        (cond
          ...
          ((compound? node) (case (compound-kind node)
            ...
            (let-values (((d ref) (project tail)))
              (unless (eq? d omitted)
                (let ((cell (last-cell chain)))
                  (set-cdr! cell d)
                  (when ref
                    (register-patch! ref tail (lambda (v) (set-cdr! cell v))))))))
          (values chain #f)))
