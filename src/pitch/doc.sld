...
;;
;; These are computed at construction, stored as a set of fullness indexes, and let the
;; resolver reject a hopeless branch without descending into it.
;;
;; NAME COLLISION. `newline` here shadows the one in (scheme write).
;; (scheme base) does not export `newline`, so a library that only builds
