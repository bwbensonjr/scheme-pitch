...
;;
;; Before this library, doc.sld and check.sld each carried their own copy, with
;; a comment in doc.sld observing that "two definitions of line ending in one
...
(begin
  ...
  (define (skip-line-ending s i n)
    (if (and (char=? (string-ref s i) #\x0d)
             (< (+ i 1) n)
             (let ((c (string-ref s (+ i 1)))) (or (char=? c #\x0a) (char=? c #\x85))))
        (+ i 2)
        (+ i 1)))
    ...
    (let ((n (string-length s)))
      (if (= n 0)
          ...
          (let ((last (string-ref s (- n 1))))
            (cond
              ((or (char=? last #\x0a) (char=? last #\x85))
                (if (and (>= n 2) (char=? (string-ref s (- n 2)) #\x0d))
                    (substring s 0 (- n 2))
                    (substring s 0 (- n 1))))
              ((or (char=? last #\x0d) (char=? last #\x2028) (char=? last #\x2029))
                (substring s 0 (- n 1)))
              (else s))))))))
