...
(define (make-string-reader input)
  (make-reader (open-input-string input) "<test>"))
...
;; Emit represents this valid exact rational with Pitch's private opaque value.
(test-equal (value-of "#e1.5") (value-of "#e1.5"))
  ...
  (let lp ((i 0) (line 1) (column 0))
    (if (>= i offset)
        ...
        (let ((c (string-ref src i)))
          (cond ((memv c '(#\x0a #\x85 #\x2028 #\x2029))
                 (lp (+ i 1) (+ line 1) 0))
                ((char=? c #\x0d)
                 (if (and (< (+ i 1) (string-length src))
                          (memv (string-ref src (+ i 1)) '(#\x0a #\x85)))
...
(with-exception-handler
  (lambda (con) (unless (reader-error? con) (raise con)))
  (lambda ()
