#!/usr/bin/env -S chez --libdirs src:. --program
...
;; (pitch doc) exports `newline`, the document constructor. The console one is
;; renamed rather than excluded, since this driver needs both.
(import
  (except (rnrs (6)) newline)
  (rename (only (rnrs io simple (6)) newline) (newline emit-newline))
  (pitch doc)
  (pitch cost)
  (pitch layout))
  ...
  (cond
    ((symbol? e)
     (case e
       ...
       ((empty-doc) empty-doc)
       ((fail) fail)
       (else (assertion-violation 'build "unknown document atom" e))))
    ((pair? e)
     (let ((head (car e)) (args (cdr e)))
       (case head
         ...
         (else (assertion-violation 'build "unknown document form" e)))))
    (else (assertion-violation 'build "not a document expression" e))))
...
(define (read-corpus path)
  (let* ((port (open-input-file path))
         (entries (get-datum port))
         (trailing (get-datum port)))
    (close-port port)
    (unless (eof-object? trailing)
      (assertion-violation 'read-corpus
        "the corpus file must hold exactly one list of entries; found data after it"
        path trailing))
    (unless (list? entries)
      (assertion-violation 'read-corpus "the corpus is not a list" path))
    entries))

(define (run-entry entry)
  (let* ((name (car entry))
         (page-width (cadr entry))
         (computation-width (caddr entry))
         (offset (cadddr entry))
         (document (car (cddddr entry)))
...
(let ((entries (read-corpus corpus-path)))
  (display "entries ") (write (length entries)) (emit-newline)
