...
the generic shape, except under a `'`, where the fallback is the packing one
instead: a quoted position changes which fallback applies and does not suppress
lookup, so a data table packs and quoted code goes on looking like code. A style
never raises, never drops an element, and never moves
