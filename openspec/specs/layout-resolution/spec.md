# layout-resolution Specification

## Purpose

The engine: a pure function from a document, a cost factory and a starting
offset to the cheapest layout the document denotes, together with its cost and
whether the search proved minimality. Two outcomes are kept distinct. A document
with no layout raises rather than inventing a best-effort rendering. A document
whose layouts all pass the computation width renders normally and reports itself
tainted, having traded the optimality claim for a bound on the search.
Resolution terminates on every document, memoizes within a single call and never
across calls, and performs no I/O. The port is differentially tested against the
Racket implementation it derives from, over a corpus that drives both sides from
one file and reports how many entries it compared.
## Requirements
