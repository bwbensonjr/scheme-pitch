...
token text carries the newline that terminated it; `verbatim` is what a caller
holding a string that legally contains one uses instead, splitting at the endings
and adding no indentation to what follows them. Construction simplifies where
