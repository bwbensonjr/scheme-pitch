# cli-output-disposition Specification

## Purpose

Where the formatted text goes, and what happens to a file the formatter will not
format. Exactly one disposition is in effect per run -- rewriting in place by
default, `--stdout`, or `--check` -- and naming two is an error rather than a
silent preference for one. A file is written only when its formatted text differs
from the text read, because formatting is idempotent and a run over an
already-formatted tree must be a no-op at the filesystem level: no file replaced,
no modification time changed, no rebuild storm, and no confusing the no-op case
with the working one. A file the pipeline refuses is left byte-identical, and no
partial or repaired form is emitted under any status. The write goes to a
temporary in the target's own directory and is renamed over it, so the rename
stays within one filesystem and an interrupted run leaves the file wholly
unchanged or wholly replaced; this is the only operation in the system that can
destroy a user's source. Standard output carries formatted text and nothing else
-- no separator, header, or filename banner -- so that redirecting it yields a
file containing only source, and every diagnostic goes to standard error. A
multi-file run continues past a failure, because the good files still need
formatting and the bad ones still need naming.
## Requirements
