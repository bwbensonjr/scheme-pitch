# cst-round-trip Specification

## Purpose

Serializing a CST back to text by concatenating its leaves in source order,
reproducing the parsed input byte for byte. The guarantee covers every construct
the reader accepts and holds for malformed input as well as well-formed input.
The check compares the serialized output against the original input text, never
against another artifact derived from the same tree.
## Requirements
