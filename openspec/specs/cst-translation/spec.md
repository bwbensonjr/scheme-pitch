...
malformed input, and it reads only token text and the child sequence -- never a
recorded offset, line or column, so a bug in the positions cannot misplace
anything. Every token's text reaches the document verbatim, exactly once and in
order, because the declared-normalizations list is empty and nothing here is
entitled to respell a lexeme. A token that legally spans lines keeps its interior
untouched and unindented. Compound nodes take one of three shapes, chosen by the
cost objective over the whole document rather than greedily, and which shape a
form takes is decided at exactly one point, so that a per-form style table
becomes a lookup rather than a rewrite.
## Requirements
...
node, regardless of its head. Until a style table exists, no per-form knowledge
lives in the translation at all.
