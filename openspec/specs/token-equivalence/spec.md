# token-equivalence Specification

## Purpose

The layer 1 safety check: comparing two source texts by tokenizing each and
comparing what remains after whitespace is filtered. Tokens are compared by kind
and text, never by position or parsed value. Code tokens and comments form one
interleaved sequence, so a comment cannot migrate across a code token unnoticed.
A single trailing line ending is dropped from a token's text before comparison,
which changes no comment content and so declares no normalization. Layer 1 is
strictly stronger than datum equivalence: it catches deleted comments, moved `#;`
datum comments, flipped brackets, expanded abbreviations, respelled lexemes, and
tokens that merged or were swallowed. A failed comparison reports the first
differing index and both tokens; a diagnostic on either text fails the check
rather than being compared past.
## Requirements
