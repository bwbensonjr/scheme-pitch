# cli-file-selection Specification

## Purpose

What the program will read, and how it decides. Standard input is selected only
when it is named -- by the operand `-`, or by `--stdout` with no operand --
because an absent operand means a caller lost its file list, not that it wanted a
stream, and `-` may not be mixed with file operands lest one run carry two
dispositions announced by no flag. A named file is formatted whatever its
extension, since naming it is a stronger signal than its suffix. A named
directory is walked, and there the extension set is a discovery filter and
nothing else: never a dialect signal, because both standards use the same
suffixes. The walk is depth-first in name order so that the sequence of files,
and therefore the order of diagnostics, reproduces across machines and runs.
Dot-named entries are skipped, which keeps the walk out of version-control and
editor directories without naming any of them. A symbolic link to a directory is
not descended into: cycle detection needs path identity the host does not expose,
and skipping is the refuse-rather-than-guess behavior the project applies
elsewhere. A path that does not exist or cannot be read is reported and the
remaining operands are still processed -- it is a fact about the invocation or
the environment, not about the formatting of any code.
## Requirements
