# datum-equivalence Specification

## Purpose

The named datum equivalence operation and the layer 2 safety check built on it.
The check accepts two source texts, parses and projects each independently, and
reports whether their data are equivalent; a diagnostic on either side fails the
check rather than being compared past. Layer 2 is strictly weaker than token
equivalence and states that weakness explicitly - its value is independence from
that check, not strength. When verifying a formatting run the second text is the
formatter's output re-read as text, never a value derived from the printer's own
tree. A host implementation's reader appears only as a differential test oracle.
## Requirements
