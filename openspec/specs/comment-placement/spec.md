...
Where a comment goes when the line it was written on no longer exists. This is
the problem every datum-based Scheme pretty-printer has been sunk by, so it is
specified as behavior rather than left to emerge from a cost function. A line
comment is always followed by a line break -- guarded twice in the printer, and a
third time by the algebra, where a hard break cannot be flattened, making a
comment that swallows the code after it unrepresentable rather than merely
forbidden. A comment written after code on its line stays on that line; one
written on its own line stays on its own line; and no comment ever crosses a code
token, because a comment that moves past code documents different code, which is
