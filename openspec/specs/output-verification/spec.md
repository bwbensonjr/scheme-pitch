...
The combined runner over the safety checks that compare two source texts: token
equivalence and datum equivalence. It accepts one pair of texts, reports whether
the pair passed, and names which layer failed so a caller need not re-run the
individual checks. Token equivalence runs first because it is strictly stronger
and reports where it failed. A text that cannot be tokenized, parsed, or
projected fails before any layer runs and is not blamed on one. The runner is
deliberately minimal: round-trip compares a tree against its own input rather
than two texts, and idempotence requires running the formatter more than once, so
both are outside it and join the pipeline with the formatter.
## Requirements
